...
export const EvaluationRekap: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="space-y-6">
            ...
            <div className="flex flex-col lg:flex-row gap-6">
              ...
              <div className="w-full lg:w-[61.8%] space-y-6">
                ...
                {/* Period Comparisons (naik/turun) */}
                <div className="p-5 border border-slate-200/60 rounded-xl bg-white space-y-4 shadow-sm">
                  <h4 className="text-xs font-bold text-slate-800 uppercase tracking-wider">
                    Perbandingan Domain Antar Periode
                  </h4>
                  
                  {rekapData.comparison_trend && rekapData.comparison_trend.length > 0 ? (
                    <div className="divide-y divide-slate-100">
                      {rekapData.comparison_trend.map((trend, idx) => (
                        <div key={idx} className="py-2.5 flex items-center justify-between gap-4">
                          <div>
                            <span className="text-sm font-semibold text-slate-700">{trend.name}</span>
                            <span className="text-xs text-slate-400 block mt-0.5">
                              Sebelumnya: {trend.prev.toFixed(2)} &rarr; Terbaru: {trend.curr.toFixed(2)}
                            </span>
                          </div>
                          
                          <div>
                            {trend.status === 'naik' ? (
                              <span className="px-2.5 py-1 rounded-full text-xs font-bold bg-green-50 text-green-700 border border-green-200 inline-flex items-center gap-1">
                                <TrendingUp className="w-3 h-3" />
                                <span>+{trend.diff.toFixed(2)} Meningkat</span>
                              </span>
                            ) : trend.status === 'turun' ? (
                              <span className="px-2.5 py-1 rounded-full text-xs font-bold bg-rose-50 text-rose-700 border border-rose-200 inline-flex items-center gap-1">
                                ...
                                <span>-{trend.diff.toFixed(2)} Menurun</span>
                              </span>
                            ) : (
                              <span className="px-2.5 py-1 rounded-full text-xs font-bold bg-slate-100 text-slate-500 border border-slate-200 inline-flex items-center gap-1">
                    ...
                    <div className="text-center py-6 text-slate-400 text-xs font-semibold">
                      Butuh minimal 2 kali penilaian untuk membandingkan perbedaan perkembangan antar periode.
                    </div>
                  )}
                </div>
              </div>
      ...
      {rekapData && rekapData.comparison_data && rekapData.comparison_data.length > 0 && (
        <div className="bg-white border border-slate-200/60 rounded-2xl p-6 shadow-sm space-y-4">
          <div className="flex items-center gap-3 border-b border-slate-100 pb-3">
            ...
            <div>
              <h2 className="text-base font-bold text-slate-800">Matriks Perbandingan Predikat Antar Siswa</h2>
              <p className="text-xs text-slate-400 font-medium">Tabel komparasi predikat perkembangan seluruh siswa per domain dari penilaian terbaru</p>
            </div>
          </div>
            ...
            <table className="w-full text-left border-collapse text-xs">
              <thead>
                <tr className="bg-slate-50 text-slate-500 font-bold border-b border-slate-100">
                  ...
                  <th className="px-5 py-3 text-center">Kognitif</th>
                  <th className="px-5 py-3 text-center">Motorik</th>
                  <th className="px-5 py-3 text-center">Bahasa</th>
                  <th className="px-5 py-3 text-center">Sosial</th>
                  <th className="px-5 py-3 text-center">Kemandirian</th>
                  <th className="px-5 py-3 text-center bg-slate-50/50">Skor Komposit</th>
                  <th className="px-5 py-3 text-center bg-slate-50/50">Hasil Predikat</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 text-sm font-semibold text-slate-700">
                {rekapData.comparison_data.map((item) => (
                  <tr key={item.student_id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-5 py-3.5">
                      ...
                      <div className="text-[10px] text-slate-400 mt-0.5">ID: #{item.student_id}</div>
                    </td>
                    <td className="px-5 py-3.5">
                    ...
                    {item.latest_evaluation ? (
                      <>
                        {/* Domain columns - we display predicate derived from average index */}
                        {/* Note: BK router comparison API computes domain averages which we can map to BB/MB/BSH/BSB. */}
                        {/* Since API doesn't return domain averages inside comparison_data, we can show indicators check, */}
                        {/* but wait! The API doesn't return average domain for comparison data. Let's verify. */}
                        {/* Ah! The backend returns:
                            latest_evaluation: { id, date, composite_index, predicate }
                            Wait, does it return the domain scores in comparison_data? Let's check:
                            In backend router:
                            latest_ev = db.query(Evaluation).filter(Evaluation.student_id == s.id)...
                            It returns latest_evaluation dict containing only:
                            "id", "date", "composite_index", "predicate".
                            Wait, so ev.avg_cognitive etc are not in the dictionary! Let's double check evaluations.py.
                            Line 530:
                            "latest_evaluation": {
                                "id": latest_ev.id,
                            ...
                            Oh! The backend doesn't return domain averages in latest_evaluation.
                            Wait! If so, let's look at the old Jinja table:
                            Line 229:
                            {% set ev = item.latest_evaluation %}
                            And then:
                            ev.avg_cognitive, ev.avg_motoric, etc.
                            Ah! In Jinja, `item.latest_evaluation` is an SQLAlchemy model object directly!
                            So Jinja could access `ev.avg_cognitive`.
                            But in the REST API, it is serialized. And the REST API serializes it as:
                            "latest_evaluation": {
                                "id": latest_ev.id,
                                "date": latest_ev.date.isoformat(),
                                "composite_index": latest_ev.composite_index,
                            ...
                            Wait, does it mean the REST API is missing the domain averages?
                            Let's check if the REST API has them. No, looking at lines 530-535:
                            latest_ev.avg_cognitive is NOT in the serialized dictionary.
                            Let's fix the backend REST API in evaluations.py first, or should we include it?
                            Yes! Let's look at lines 523-536 in evaluations.py:
                            "latest_evaluation": {
                                "id": latest_ev.id,
                                "date": latest_ev.date.isoformat(),
                                "composite_index": latest_ev.composite_index,
                        ...
                        <td className={`px-5 py-3.5 text-center font-bold text-xs ${
                          item.latest_evaluation.avg_cognitive && item.latest_evaluation.avg_cognitive >= 3.5 ? 'text-green-600' :
                          item.latest_evaluation.avg_cognitive && item.latest_evaluation.avg_cognitive >= 2.5 ? 'text-blue-600' :
                          item.latest_evaluation.avg_cognitive && item.latest_evaluation.avg_cognitive >= 1.5 ? 'text-amber-600' :
                          item.latest_evaluation.avg_cognitive ? 'text-rose-600' : 'text-slate-500'
                        }`}>
                          {item.latest_evaluation.avg_cognitive ? (
                            item.latest_evaluation.avg_cognitive >= 3.5 ? 'BSB' :
                            item.latest_evaluation.avg_cognitive >= 2.5 ? 'BSH' :
                            item.latest_evaluation.avg_cognitive >= 1.5 ? 'MB' : 'BB'
                          ) : '—'}
                        </td>
                        <td className={`px-5 py-3.5 text-center font-bold text-xs ${
                          item.latest_evaluation.avg_motoric && item.latest_evaluation.avg_motoric >= 3.5 ? 'text-green-600' :
                          item.latest_evaluation.avg_motoric && item.latest_evaluation.avg_motoric >= 2.5 ? 'text-blue-600' :
                          item.latest_evaluation.avg_motoric && item.latest_evaluation.avg_motoric >= 1.5 ? 'text-amber-600' :
                          item.latest_evaluation.avg_motoric ? 'text-rose-600' : 'text-slate-500'
                        }`}>
                          {item.latest_evaluation.avg_motoric ? (
                            item.latest_evaluation.avg_motoric >= 3.5 ? 'BSB' :
                            item.latest_evaluation.avg_motoric >= 2.5 ? 'BSH' :
                            item.latest_evaluation.avg_motoric >= 1.5 ? 'MB' : 'BB'
                          ) : '—'}
                        </td>
                        <td className={`px-5 py-3.5 text-center font-bold text-xs ${
                          item.latest_evaluation.avg_language && item.latest_evaluation.avg_language >= 3.5 ? 'text-green-600' :
                          item.latest_evaluation.avg_language && item.latest_evaluation.avg_language >= 2.5 ? 'text-blue-600' :
                          item.latest_evaluation.avg_language && item.latest_evaluation.avg_language >= 1.5 ? 'text-amber-600' :
                          item.latest_evaluation.avg_language ? 'text-rose-600' : 'text-slate-500'
                        }`}>
                          {item.latest_evaluation.avg_language ? (
                            item.latest_evaluation.avg_language >= 3.5 ? 'BSB' :
                            item.latest_evaluation.avg_language >= 2.5 ? 'BSH' :
                            item.latest_evaluation.avg_language >= 1.5 ? 'MB' : 'BB'
                          ) : '—'}
                        </td>
                        <td className={`px-5 py-3.5 text-center font-bold text-xs ${
                          item.latest_evaluation.avg_social && item.latest_evaluation.avg_social >= 3.5 ? 'text-green-600' :
                          item.latest_evaluation.avg_social && item.latest_evaluation.avg_social >= 2.5 ? 'text-blue-600' :
                          item.latest_evaluation.avg_social && item.latest_evaluation.avg_social >= 1.5 ? 'text-amber-600' :
                          item.latest_evaluation.avg_social ? 'text-rose-600' : 'text-slate-500'
                        }`}>
                          {item.latest_evaluation.avg_social ? (
                            item.latest_evaluation.avg_social >= 3.5 ? 'BSB' :
                            item.latest_evaluation.avg_social >= 2.5 ? 'BSH' :
                            item.latest_evaluation.avg_social >= 1.5 ? 'MB' : 'BB'
                          ) : '—'}
                        </td>
                        <td className={`px-5 py-3.5 text-center font-bold text-xs ${
                          item.latest_evaluation.avg_independence && item.latest_evaluation.avg_independence >= 3.5 ? 'text-green-600' :
                          item.latest_evaluation.avg_independence && item.latest_evaluation.avg_independence >= 2.5 ? 'text-blue-600' :
                          item.latest_evaluation.avg_independence && item.latest_evaluation.avg_independence >= 1.5 ? 'text-amber-600' :
                          item.latest_evaluation.avg_independence ? 'text-rose-600' : 'text-slate-500'
                        }`}>
                          {item.latest_evaluation.avg_independence ? (
                            item.latest_evaluation.avg_independence >= 3.5 ? 'BSB' :
                            item.latest_evaluation.avg_independence >= 2.5 ? 'BSH' :
                            item.latest_evaluation.avg_independence >= 1.5 ? 'MB' : 'BB'
                          ) : '—'}
                        </td>
                        <td className="px-5 py-3.5 text-center bg-slate-50/30 font-bold text-slate-800">
                          {item.latest_evaluation.composite_index.toFixed(2)}
                        </td>
                        <td className="px-5 py-3.5 text-center bg-slate-50/30">
                          <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${
                            item.latest_evaluation.predicate === 'BSB' ? 'bg-green-50 text-green-700 border border-green-200' :
                            item.latest_evaluation.predicate === 'BSH' ? 'bg-blue-50 text-blue-700 border border-blue-200' :
                            item.latest_evaluation.predicate === 'MB' ? 'bg-amber-50 text-amber-700 border border-amber-200' :
                            'bg-rose-50 text-rose-700 border border-rose-200'
                          }`}>
                            {item.latest_evaluation.predicate}
                          </span>
                        </td>
                      </>
                    ) : (
                      <td colSpan={7} className="px-5 py-3.5 text-center text-slate-400 italic text-[11px] font-medium">
                        Belum dinilai
                      </td>
                    )}
                  </tr>
